...
export function NextActions({ data }: { data: MetasData }) {
  ...
  return (
    <Card className="flex h-full flex-col bg-[oklch(96.5%_0.02_322)] px-5 pb-4 pt-4">
      <div className="flex items-center gap-3.5">
        <IconBox size="md">
          <Flag className="h-5 w-5" />
        </IconBox>
        <h2 className="flex-1 text-base font-semibold">Próximas metas / Ações</h2>
        <span className="rounded-full bg-primary/15 px-3 py-1 text-xs font-medium text-primary-deep">
          {pending === 0 ? "Tudo em dia" : `${pending} ${pending === 1 ? "pendência" : "pendências"}`}
        </span>
      </div>

      <ul className="mt-2 flex flex-1 flex-col justify-around divide-y divide-border/70">
        {rows.map((row) => (
          <li key={row.key} className="flex flex-1 items-center gap-3 py-2">
            ...
            <span className="shrink-0 rounded-full border border-primary/15 bg-white/80 px-3 py-1 text-[11px] font-medium text-primary-deep">
